...
void PlotRapPt(int ptBinMin, int ptBinMax, int rapBin, double yMin, double yMax, char yAxisTitle[200], char filename[200], double lmean[ToyMC::nPtBins], double lmeanerr_low[ToyMC::nPtBins], double lmeanerr_high[ToyMC::nPtBins], bool DrawLine, double lamline, char GraphFolder[200], char GraphName[200], bool SaveGraph, int nState, int nFrame, bool RealData, char* realdatadir, char* TreeID){
	...
	for(int ptBin = ptBinMin; ptBin < ptBinMax+1; ptBin++) {
		...
		ptCentre_[pt]=mean_pT;
		ptCentreErr_low[pt]=TMath::Abs(mean_pT-onia::pTRange[rapBin][ptBin-1]);
		ptCentreErr_high[pt]=TMath::Abs(mean_pT-onia::pTRange[rapBin][ptBin]);
		pt++;
	}
...
void PlotComparisonRapPt(int ptBinMin, int ptBinMax, int rapBin, double yMin, double yMax, char yAxisTitle[200], char filename[200], double lmean1[ToyMC::nPtBins], double lmean1err_low[ToyMC::nPtBins], double lmean1err_high[ToyMC::nPtBins], double lmean2[ToyMC::nPtBins], double lmean2err_low[ToyMC::nPtBins], double lmean2err_high[ToyMC::nPtBins], double lmean3[ToyMC::nPtBins], double lmean3err_low[ToyMC::nPtBins], double lmean3err_high[ToyMC::nPtBins], double lamline, int nState, bool RealData, char* realdatadir, char* TreeID){
	...
	for(int ptBin = ptBinMin; ptBin < ptBinMax+1; ptBin++) {
		...
		ptCentre_[pt]= mean_pT + mean_pT * 0.02;
		ptCentreErr_low[pt]=TMath::Abs(mean_pT-onia::pTRange[rapBin][ptBin-1]);
		ptCentreErr_high[pt]=TMath::Abs(mean_pT-onia::pTRange[rapBin][ptBin]);
		pt++;
	}
	...
	for(int iRap = rapBinMin; iRap < rapBinMax+1; iRap++){
		...
		for(int iLam=1; iLam<nLamPlots+1;iLam++){
			...
			for(int rapBin = rapBinMin; rapBin < rapBinMax+1; rapBin++) {
				...
				if(iLam<103) PlotRapPt(ptBinMin, ptBinMax, rapBin, lammin, lammax, axislabel, filename, KinDepRap, KinDepRaperr_low, KinDepRaperr_high, DrawLine, lamline, GraphFolder, GraphName, SaveGraph, nState, nFrame, RealData, realdatadir, TreeID);
				else PlotComparisonRapPt(ptBinMin, ptBinMax, rapBin, lammin, lammax, axislabel, filename, KinDepRap, KinDepRaperr_low, KinDepRaperr_high, KinDepRap2, KinDepRap2err_low, KinDepRap2err_high, KinDepRap3, KinDepRap3err_low, KinDepRap3err_high, lamline, nState, RealData, realdatadir, TreeID);
