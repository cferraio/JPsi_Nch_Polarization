...
double getMeanPt(int rapBin, int ptBin, int cpmBin, const std::string &massfile){
...
double getMeanCPM(int rapBin, int ptBin, int cpmBin, const std::string &massfile){
    RooWorkspace* ws = getFromTFile<RooWorkspace>(massfile, "ws_masslifetime");

    // Get stuff from dataset (make sure it is not 0)
    RooRealVar *JpsiMass = ws->var("JpsiMass");
    RooRealVar *JpsiCPM = ws->var("JpsiCPM");
    RooAbsData *data = ws->data(Form("data_rap%d_pt%d_cpm%d",rapBin,ptBin,cpmBin));
    assert( 0 != JpsiMass && 0 != JpsiCPM && 0 != data);

    TH1* histCPM = data->createHistogram("histCPM", *JpsiCPM, Binning(120));
    TH1* histMass = data->createHistogram("histMass", *JpsiMass, Binning(120));

    double meanCPM = histCPM->GetMean();
    double meanMass = histCPM->GetMean();
...
double getFracBkg(int rapBin, int ptBin, int cpmBin, const std::string &massfile){
    RooWorkspace* ws = getFromTFile<RooWorkspace>(massfile, "ws_masslifetime");
...
double getFracBkgIn3Sigma(int rapBin, int ptBin, int cpmBin, const std::string &massfile, int nSigMass){
    RooWorkspace* ws = getFromTFile<RooWorkspace>(massfile, "ws_masslifetime");
...
double getFracBkgErr(int rapBin, int ptBin, int cpmBin, const std::string &massfile){
...
double getFracBkgErrIn3Sigma(int rapBin, int ptBin, int cpmBin, const std::string &massfile, int nSigMass){
    TFile *inFile=new TFile(massfile.c_str(),"R");
    if(!inFile) {std::cout<<"Error: failed to opne root file"<<std::endl;}
    RooWorkspace *ws=(RooWorkspace *)inFile->Get("ws_masslifetime");
    if(!ws) {std::cout<<"Error: failed to opne workspace "<<std::endl;}
